...
const DriverEarnings = () => {
    ...
    const loadExistingEarning = async () => {
      ...
      if (existing) {
        ...
        setEarnings({
          ...
          uberSubscription: existing.uber_subscription?.toString() || "",
  ...
  useEffect(() => {
    ...
    const uberSubscription = parseFloat(earnings.uberSubscription) || 0;
    const adjustment = parseFloat(earnings.adjustment) || 0;
    const other = parseFloat(earnings.other) || 0;

    const total = weeklyEarning - cash - tax + toll - rent - uberSubscription + adjustment - other;
  ...
  const validateForm = () => {
    ...
    if (!earnings.uberSubscription || parseFloat(earnings.uberSubscription) < 0) {
      newErrors.uberSubscription = "Uber Subscription amount is required and cannot be negative";
    }
  ...
  const handleSaveEarnings = async () => {
    ...
    try {
      const earningData = {
        ...
        uber_subscription: parseFloat(earnings.uberSubscription) || 0,
...
• Uber Subscription: -₹${earnings.uberSubscription || 0}
            ...
            <CardContent className="p-8 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-3 sm:col-span-2">
                  <Label htmlFor="uberSubscription" className="text-base font-semibold flex items-center gap-2 text-gray-700">
                    Uber Subscription <span className="text-red-600 text-lg">-</span>
                    <span className="text-red-500">*</span>
                  </Label>
                  <Input
                    id="uberSubscription"
                    type="text"
                    placeholder="0"
                    value={earnings.uberSubscription}
                    onChange={(e) => handleInputChange("uberSubscription", e.target.value)}
                    className={`h-12 border-2 rounded-xl transition-all duration-200 ${errors.uberSubscription ? 'border-red-500 bg-red-50' : 'border-gray-200 focus:border-indigo-500'}`}
                    disabled={isReadOnly}
                  />
                  {errors.uberSubscription && <p className="text-red-500 text-sm font-medium">{errors.uberSubscription}</p>}
                </div>
              </div>
            ...
            <Card className="shadow-xl border-0 bg-white/90 backdrop-blur-sm">
              ...
              <CardContent className="p-6">
                <div className="space-y-4 text-base">
                  ...
                  <div className="flex justify-between items-center py-2 border-b border-gray-100">
                    <span className="font-medium">Uber Subscription</span>
                    <span className="text-red-600 font-bold">-₹{earnings.uberSubscription || 0}</span>
                  </div>
                  {earnings.adjustment && (
                    <div className="flex justify-between items-center py-2 border-b border-gray-100">
